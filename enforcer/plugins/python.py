...
class Plugin:
    ...
    def autofix_style(self, files, tool_configs=None):
        ...
        black_res = subprocess.run(
            black_cmd, capture_output=True, text=True, encoding="utf-8"
        )
        if black_res.stderr:
        ...
        isort_res = subprocess.run(
            isort_cmd, capture_output=True, text=True, encoding="utf-8"
        )
        if isort_res.stderr:
    ...
    def lint(self, files, disabled_rules, tool_configs=None):
        ...
        pyright_res = subprocess.run(
            pyright_cmd, capture_output=True, text=True, encoding="utf-8"
        )
        ...
        flake8_res = subprocess.run(
            flake8_cmd, capture_output=True, text=True, encoding="utf-8"
        )
        ...
        mypy_res = subprocess.run(
            mypy_cmd, capture_output=True, text=True, encoding="utf-8"
        )
        if mypy_res.stdout:
