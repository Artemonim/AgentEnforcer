...
def test_main_default(capsys):
    with patch("sys.argv", ["agent-enforcer"]), \
         patch("enforcer.main.load_config", return_value={}), \
         patch("enforcer.main.Enforcer") as mock_enforcer, \
         patch("enforcer.main.os.getcwd", return_value="/root"):
...
def test_main_verbose(capsys):
    with patch("sys.argv", ["agent-enforcer", "--verbose"]), \
         patch("enforcer.main.load_config", return_value={}), \
         patch("enforcer.main.Enforcer") as mock_enforcer, \
         patch("enforcer.main.os.getcwd", return_value="/root"):
        mock_instance = mock_enforcer.return_value
...
def test_main_modified(capsys):
    with patch("sys.argv", ["agent-enforcer", "--modified"]), \
         patch("enforcer.main.load_config", return_value={}), \
         patch("enforcer.main.get_git_modified_files", return_value=["mod.py"]), \
         patch("enforcer.main.Enforcer") as mock_enforcer, \
         patch("enforcer.main.os.getcwd", return_value="/root"):
        mock_instance = mock_enforcer.return_value
...
def test_main_with_root(capsys):
    with patch("sys.argv", ["agent-enforcer", "--root", "/fake_root"]), \
         patch("enforcer.main.load_config", return_value={}), \
         patch("enforcer.main.Enforcer") as mock_enforcer, \
         patch("enforcer.main.os.chdir") as mock_chdir, \
         patch("enforcer.main.os.getcwd", return_value="/fake_root"):
        mock_instance = mock_enforcer.return_value
...
def test_main_with_blacklist(capsys):
    with patch("sys.argv", ["agent-enforcer", "--blacklist", "rule1"]), \
         patch("enforcer.main.load_config", return_value={}), \
         patch("enforcer.main.save_config") as mock_save, \
...
def test_main_with_error(capsys):
    with patch("sys.argv", ["agent-enforcer", "--error", "rule1"]), \
         patch("enforcer.main.load_config", return_value={}), \
         patch("enforcer.main.save_config") as mock_save, \
         patch("sys.exit") as mock_exit:
        main()
...
def test_main_with_ignore(capsys):
    with patch("sys.argv", ["agent-enforcer", "--ignore", "rule1"]), \
         patch("enforcer.main.load_config", return_value={}), \
         patch("enforcer.main.Enforcer") as mock_enforcer, \
         patch("enforcer.main.os.getcwd", return_value="/root"):
        mock_instance = mock_enforcer.return_value
