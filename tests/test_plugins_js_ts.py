...
def test_lint_config_error(tmp_path):
    ...
    plugin = Plugin()
    with patch("enforcer.plugins.js_ts.run_command") as mock_run:
        mock_run.return_value = subprocess.CompletedProcess([], 1, stdout="", stderr="config error")
        result = plugin.lint([str(file)], [])
...
def test_lint_parse_error(tmp_path):
    ...
    plugin = Plugin()
    with patch("enforcer.plugins.js_ts.run_command") as mock_run:
        mock_run.return_value = subprocess.CompletedProcess([], 0, stdout="invalid json")
        result = plugin.lint([str(file)], [])
        assert len(result["errors"]) == 1
...
def test_compile():
    plugin = Plugin()
    with patch("enforcer.plugins.js_ts.run_command") as mock_run:
        mock_run.return_value = subprocess.CompletedProcess([], 1, stdout="error1\nerror2")
...
def test_test():
    plugin = Plugin()
    with patch("enforcer.plugins.js_ts.run_command") as mock_run:
        mock_run.side_effect = subprocess.CalledProcessError(1, ["cmd"], output="test error")
