...
def test_lint(tmp_path):
    ...
    with patch("enforcer.plugins.kotlin.run_command") as mock_run, \
        ...
        mock_run.side_effect = [
            subprocess.CompletedProcess([], 0, stdout=f"{file}:1:1: some ktlint error", stderr=""),
            subprocess.CompletedProcess([], 0, stdout=f"{file}:2:2 - some-rule - some detekt error", stderr=""),
        ]
...
def test_lint_ktlint_error(tmp_path):
    ...
    with patch("enforcer.plugins.kotlin.run_command") as mock_run, \
         patch("os.path.exists", return_value=True):
        mock_run.side_effect = [subprocess.CompletedProcess([], 0, stdout=f"{file}:1:1: error msg"), subprocess.CompletedProcess([], 0, stdout="")]
        result = plugin.lint([str(file)], [], root_path=str(tmp_path))
...
def test_lint_detekt_timeout(tmp_path):
    ...
    plugin = Plugin()
    with patch("enforcer.plugins.kotlin.run_command") as mock_run:
        mock_run.side_effect = [subprocess.CompletedProcess([], 0, ""), subprocess.TimeoutExpired(["cmd"], 10)]
